...
class Sddr(object):
    ...
    def train(self, target, structured_data, structured_data_test, unstructured_data=dict(),unstructured_data_test=dict(), unstructured_tensors = dict(), resume=False, plot=False):
        ...
        with torch.no_grad():
            ...
            self.val_individual_NLL  = torch.index_select(self.net.get_log_loss(target).to(self.device), 0, torch.tensor(test_indices).numpy())
            self.val_preds = dict()
            for param in datadict.keys():
                ...
                self.val_preds[param] = torch.index_select(vars(self.net(datadict,training=False))[param], 0, torch.tensor(test_indices).numpy())
